Процедура ОбработкаПроведения(Отказ, Режим)
	Для Каждого ТекСтрока Из СписокНоменклатуры Цикл
		...
		Выборка = ПолучитьСписокПартийСогласноУчетнойПолитике(Партия, ТекСтрока.Номенклатура, УчетнаяПолитика);
...
Функция ПолучитьСписокПартийСогласноУчетнойПолитике(ИсключаемаяПартия, Номенклатура, УчетнаяПолитика)
	...
	Запрос.Текст =
	    ...
	    |	И ОстаткиНоменклатурыОстатки.Номенклатура = &Номенклатура
	    |	И ОстаткиНоменклатурыОстатки.Партия <> &ИсключаемаяПартия
	    |
	    |УПОРЯДОЧИТЬ ПО
	    |	ОстаткиНоменклатурыОстатки.Партия.Дата " + СортировкаСогласноУчетнойПолитике;
	Запрос.УстановитьПараметр("ИсключаемаяПартия", ИсключаемаяПартия);
	Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
